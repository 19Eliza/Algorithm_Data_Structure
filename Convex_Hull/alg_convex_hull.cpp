...
namespace convex_hull{
    ...
    std::vector<Point> Convex_Hull(std::vector<Point>& p){
        ...
        for (int i = 2; i < n; i++) {
            ...
            while(L_upper.size() > 2 && Left_Rotation(*(L_upper.end()-3), *(L_upper.end() - 2), *(L_upper.end() - 1))) {
        ...
        for (int i = n-3; i > -1; i--) {
            ...
            while (L_lower.size() > 2 && Left_Rotation(*(L_lower.end() - 3), *(L_lower.end() - 2), *(L_lower.end() - 1))) {
